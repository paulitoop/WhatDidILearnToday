...
## 14.11.2024
1. Интересная статья про уязвимость в *Google Chrome*, которая использует платежи через *Service Worker* и позволяет реализовать xss, если на конечном ресурсе реализован функционал загрузки и скачивания файлов 
2. Построение L3 vPN с помощью инкапсуляции GRE (незащищенный протокол)
3. Любой хороший рассказ, призванный донести мысль, можно построить по **пирамиде Минто**. Ситуация - Проблема - Вопрос - Ответ - Аргументы - Заключение
4. Узнал о классах защищенности к1 к2 и к3
Например, в к3 система замыкается путем подсчета контрольных сумм. Любая установка бинарников ведет к блокировке системы.

## 12.11.2024
1.T-Банк Let's gooooo

## 9.11.2024
1. Узнал про надстройку для Kubernets 'Crossplane'. Она служит для удобного выращивания api слоя соединяя воедино разных провайдеров.
2. Понял что со 'Spark' намного более удобно работать через SQL а не sparkapi. По крайне мере, синтаксис более понятен и читабелен
3. Если две строки не равны между собой, хотя должны, надо проверять английские буквы :)
4. Прямой вызов 'powershell' скрипта ведет к вызову синего окна, пускай и на короткое время.
5. Узнал что с помощью 'ssh' можно не создавать сессию, а выполнить одну команду на удаленном хосте.
6. Узнал про 'RDD' - низкоуровневый язык для работы с неструктурированными данными в 'Spark'
...
5. С помощью `kill` в `bash` можно послать любой сигнал. Данные о сигналах в файле `/usr/include/asm-generic/signal.h`
6. Для указания процесса в `kill` можно использовать pid, id (%n), название (%s), подстроку (?%s) и тд, короче МНОГО всего
7. Взаимодействие с фоновыми процессами возможно с помощью `COPROC`
8. `nohup` - процесс не привязывается к терминалу. `disown` - процесс отвязывается от терминала
